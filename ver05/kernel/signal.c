...
int sys_sigpending(sigset_t *set)
{
    ...
    verify_area(set,4);
    put_fs_long(current->blocked & current->signal, (unsigned int *)set);
    return 0;
}
 ...
 */
int sys_sigsuspend(int restart, unsigned int old_mask, unsigned int set)
{
 ...
 */
int core_dump(int signr)
{
	return(0);	/* We didn't do a dump */
}

int do_signal(int signr,int eax,int ebx, int ecx, int edx, int orig_eax,
	int fs, int es, int ds,
	int eip, int cs, int eflags,
	unsigned int * esp, int ss)
{
	...
	*(&esp) -= longs;
	...
	tmp_esp=esp;
	put_fs_long((int) sa->sa_restorer,tmp_esp++);
