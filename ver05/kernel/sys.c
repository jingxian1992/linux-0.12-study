...
extern void system_info_init(void)
{
	...
	{
		strcpy(thisname.sysname, UTS_SYSNAME);
	}
	
	i = strlen(UTS_NODENAME);
	if (i > 63)
	{
		strcpy(thisname.nodename, "(none)");
	}
	else
	{
		strcpy(thisname.nodename, UTS_NODENAME);
	}

	i = strlen(UTS_RELEASE);
	if (i > 63)
	{
		strcpy(thisname.release, "0");
	}
	else
	{
		strcpy(thisname.release, UTS_RELEASE);
	}
	
	i = strlen(UTS_VERSION);
	if (i > 63)
	{
		strcpy(thisname.version, "0.12");
	}
	else
	{
		strcpy(thisname.version, UTS_VERSION);
	}
	
	i = strlen(UTS_MACHINE);
	if (i > 63)
	{
		strcpy(thisname.machine, "i386");
	}
	else
	{
		strcpy(thisname.machine, UTS_MACHINE);
	}
}
