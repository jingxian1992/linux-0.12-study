 ...
 */
int sys_select( unsigned int *buffer )
{
/* Perform the select(nd, in, out, ex, tv) system call. */
	...
	timeout = 0xffffffff;
	if (tvp) {
		timeout = get_fs_long((unsigned int *)&tvp->tv_usec)/(1000000/HZ);
		timeout += get_fs_long((unsigned int *)&tvp->tv_sec) * HZ;
		timeout += jiffies;
	}
	...
	if (tvp) {
		...
		put_fs_long(timeout/HZ, (unsigned int *) &tvp->tv_sec);
		timeout %= HZ;
		timeout *= (1000000/HZ);
		put_fs_long(timeout, (unsigned int *) &tvp->tv_usec);
	}
